...
>
> **Status:** [x] done — Steps 1–9 complete; `main.go` wiring (Step 6, second half) lands with Task 18; Step 10 (live smoke) deferred.
...
- [x] **Step 6: Wire it through config and `main.go` (amends Task 2 & Task 18)** — `config.go` fields + `LLMExtractorConfig()` done; the `run()` block is applied when Task 18 is built (forward-referenced there).
