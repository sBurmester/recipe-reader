...
> **Teilweise erledigt.** Zwei der drei Unterpunkte des folgenden Eintrags sind im Plan [2026-09-22-release-and-cleanup.md](2026-09-22-release-and-cleanup.md) behoben (M3) und hier gestrichen: ein abgebrochener Migrationslauf sagt, dass nichts halb angewandt ist und ein erneuter Lauf fortsetzt, und `main` meldet eine abgelehnte Kommandozeile als `invalid command line` statt als `fatal`. Offen bleibt kongs Kommandopfad-Präfix, weil es ein Nutzerurteil braucht.
...
> **Erledigt.** Der folgende Eintrag ist im Plan [2026-09-22-release-and-cleanup.md](2026-09-22-release-and-cleanup.md) umgesetzt (M1, Entscheidung E3): der Advisory-Lock hält eine eigene `pgx.Connect`-Verbindung statt einer aus dem Pool, sodass `pool.Close()` in `server.Run` nicht mehr auf einen aufgegebenen Import wartet. Offen bleibt die Ursache: ein aufgegebener Lauf endet weiterhin erst nach bis zu einem `LLM_TIMEOUT`; das steht im Backlog jenes Plans.
...
> **Erledigt.** Der folgende Eintrag ist im Plan [2026-09-22-release-and-cleanup.md](2026-09-22-release-and-cleanup.md) umgesetzt (M2, Entscheidung E4): der `app`-Service liest `.env` per `env_file:`, sodass jede Variable daraus den Container erreicht. Dabei fiel auf, dass Compose das host-seitige `DB_DSN` aus `.env` (auf `localhost`) auch in den Container substituierte; der Container-DSN wird deshalb immer aus `POSTGRES_*` gebaut und ist über `CONTAINER_DB_DSN` überschreibbar.
