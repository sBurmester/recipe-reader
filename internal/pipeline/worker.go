...
// is already in progress, or Instagram is still being waited out after a rate
// limit, it returns the previous result immediately without starting a second
// one. The pipeline runs outside the lock so that Status stays responsive
...
func (w *Worker) RunOnce(ctx context.Context) ImportResult {
	...
	if remaining := time.Until(w.cooldownUntil); remaining > 0 {
		...
		// looks identical to one that ran and found nothing, and the tally it
		// returns is the previous run's.
