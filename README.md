...
1. Add one file, `internal/db/migrations/000N_<name>.sql`, holding both directions:
   ...
   file instead. Keep the annotations out of comments as well: goose reads any comment line that
   contains one as a directive, mid-sentence or not, and refuses the whole file.
...
rows, write the bookkeeping by hand once. The failed start has usually created `goose_db_version`
already, holding only version 0, so the statements accept a table that is there:
    ...
    INSERT INTO goose_db_version (version_id, is_applied)
        SELECT v, true FROM generate_series(0, 3) AS v
        WHERE NOT EXISTS (SELECT 1 FROM goose_db_version WHERE version_id = v);
