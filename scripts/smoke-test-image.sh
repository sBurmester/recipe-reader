...
# The image's own HEALTHCHECK — the binary probing itself with its healthcheck
# command — is reached by no unit test. A renamed command, a wrong binary path
# or a probe that cannot dial the listen address would leave every check above
# passing while Docker reports the container unhealthy forever.
health=
